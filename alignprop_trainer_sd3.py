...
class AlignPropTrainerSD3(PyTorchModelHubMixin):
    """
    ...
    Attributes:
        ...
        sd_pipeline (`AlignPropDiffusionPipeline3`):
    ...
    def __init__(self, config: AlignPropConfig, prompt_function: Callable[[], Tuple[str, Any]],
                 sd_pipeline: AlignPropDiffusionPipeline3,
        ...
        self.sd_pipeline.transformer.to(self.accelerator.device, dtype=inference_dtype)
        ...
        neg_tok = self.sd_pipeline.tokenizer(
        ...
        print("neg_tok.shape : ",neg_tok.shape)
        self.neg_prompt_embed = self.sd_pipeline.text_encoder(neg_tok)[0]
        print("self.neg_prompt_embed.shape", self.neg_prompt_embed.shape)
        ...
        if hasattr(self.sd_pipeline, "use_lora") and self.sd_pipeline.use_lora:
            transformer, self.optimizer = self.accelerator.prepare(trainable_layers, self.optimizer)
            self.trainable_layers = list(filter(lambda p: p.requires_grad, transformer.parameters()))
        else:
    ...
    def step(self, epoch: int, global_step: int):
        ...
        self.sd_pipeline.transformer.train()

        for _ in range(self.config.train_gradient_accumulation_steps):
            with self.accelerator.accumulate(self.sd_pipeline.transformer), self.autocast(), torch.enable_grad():
    ...
    def _generate_samples(self, batch_size, with_grad=True, prompts=None):
        ...
        prompt_embeds = self.sd_pipeline.text_encoder(prompt_ids.to(self.accelerator.device), output_hidden_states=True)

        pooled_prompt_embeds = prompt_embeds[0]
        prompt_embeds = prompt_embeds.hidden_states[-2]
        prompt_embeds = prompt_embeds.to(dtype=self.sd_pipeline.text_encoder.dtype, device=self.accelerator.device)
